...
export class TrackerAdminService {
  ...
  patchCategory(category: Category): Observable<any> {
    return this.http.patch(`${this.baseUrl}/categories/${category.id}`, category);
  }

  createRule(payload: { categoryId: string; pattern: string }): Observable<any> {
    return this.http.post(`${this.baseUrl}/rules`, payload);
  }

  getRules(): Observable<CategoryRule[]> {
    return this.http.get<CategoryRule[]>(`${this.baseUrl}/rules`);
  }
  ...
  recalculate(): Observable<any> {
    return this.http.post(`${this.baseUrl}/recalculate`, {})
  }

  importDefaultTemplates(): Observable<any> {
    return this.http.post(`${this.baseUrl}/categories/import-default`, {})
  }
